...
#############################################################split the input fastq######################################################################
	...
	print file "cp -r $AS","_S1_$sample[$j]_R*_001.fastq.gz $workdir","split/$sample[$j]\n";
	print file "cd $workdir","split/$sample[$j]\n";
	print file "gunzip $AS","_S1_$sample[$j]_R1_001.fastq.gz\n";
	print file "gunzip $AS","_S1_$sample[$j]_R2_001.fastq.gz\n";
	print file "split -l $nspl $AS","_S1_$sample[$j]_R1_001.fastq $sample[$j]_1_split_\n";
	print file "split -l $nspl $AS","_S1_$sample[$j]_R2_001.fastq $sample[$j]_2_split_\n";
	print file "rm $AS","_S1_$sample[$j]_R1_001.fastq\n";
	print file "rm $AS","_S1_$sample[$j]_R2_001.fastq\n";
	close(file);
