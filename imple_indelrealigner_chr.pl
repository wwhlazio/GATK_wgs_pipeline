...
my $sh_file=$workdir."indelrealigner_by_chr.sh";
print $sh_file,"\n";
open(my $FILE,">",$sh_file) or die"I can't write to the file!";
for my $i(0..$nchr-1){
	my $lsf_file=$workdir."indelrealigner_".$chr[$i].".lsf";
	...
	print $FILEX "#BSUB -o $workdir","indelrealigner_$chr[$i]",".stdout\n";
	print $FILEX "#BSUB -eo $workdir","indelrealigner_$chr[$i]",".stderr\n";
	print $FILEX "#BSUB -L /bin/bash\n";
	print $FILEX "module load java/1.8.0_66\n";
	print $FILEX "java -jar -Xmx4g /hpc/packages/minerva-common/gatk/3.6-0/src/GenomeAnalysisTK.jar -T IndelRealigner -R /sc/orga/projects/zhuj05a/Wenhui/IBD/data/hg38/hg38.fa -I $datadir","$input -known /sc/orga/projects/zhuj05a/Wenhui/IBD/data/hg38/Mills_and_1000G_gold_standard.indels.hg38.vcf.gz -targetIntervals $datadir","Reads_1.$chr[$i].realigner.intervals -o $datadir","alignment_KY16K404.sorted.rmdup.good.realign.",$chr[$i],".bam\n";
	close($FILEX);
