...
#	ub_pipe_line_new($integ_step1,$workdir,$fp[$i],$mem1,$mem2,$time1,$time2,$data_split,$nsf,"integ_step1","integ_step1",@fps);
...
sub sub_pipe_line{
	...
	for $i(9..scalar(@_)-1){
		...
		print filex "rm $workdir","unmapped/$fp/$fps[$j]/unmapped*\n";
                ...
                print filex "perl $vf_bin","preprocess_alignonly.pl -c $cff -o $workdir","unmapped/$fp/$fps[$j]/\n";
		print filex "rm $workdir","unmapped/$fp/$fps[$j]/alignment.sam\n";
                close(filex);
